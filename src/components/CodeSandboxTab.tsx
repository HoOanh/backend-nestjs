...
export const CodeSandboxTab: React.FC<CodeSandboxTabProps> = ({ lesson, onLessonCompleted }) => {
  ...
  const updateSuggestions = (text: string, pos: number) => {
    ...
    // Calculate approximate caret position for popup
    const linesBefore = leftText.split('\n');
    const currentLineNumber = linesBefore.length;
    const currentColumn = linesBefore[linesBefore.length - 1].length;

    const top = Math.min(currentLineNumber * 22 + 10, 260);
    const left = Math.min(currentColumn * 8.5 + 50, 400);
  ...
  const applySuggestion = (item: SuggestionItem) => {
    ...
    const newCode = code.slice(0, cursorWordRange.start) + insert + code.slice(cursorWordRange.end);
    setCode(newCode);
    localStorage.setItem(storageDraftKey, newCode);

    setSuggestions([]);
    const nextCursor = cursorWordRange.start + insert.length;
    setTimeout(() => {
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    ...
    // Standard Tab key handling (insert 2 spaces)
    if (e.key === 'Tab') {
      e.preventDefault();
      const target = e.currentTarget;
  ...
  return (
    <div className="sandbox-card">
      <div className="sandbox-header">
        <div className="sandbox-header-badge">Thực Hành & Chấm Điểm Sandbox</div>
      ...
      <div className="vs-code-editor sandbox-ide">
        <div className="vs-editor-titlebar">
          ...
          <div className="vs-editor-tabs">
            <div className="vs-tab active">
              ...
              <span className="vs-tab-title">solution.ts</span>
            </div>
          </div>
          <div className="vs-editor-actions">
            <span className="vs-breadcrumb-text">src &gt; exercises &gt; {lesson.id} &gt; solution.ts</span>
        ...
        <div className="sandbox-editor-wrapper">
          ...
          <div className="sandbox-code-area">
            ...
            <textarea
              ...
              spellCheck="false"
              placeholder="// Viết code giải bài tập TypeScript tại đây..."
            />
        ...
        <div className="sandbox-editor-footer">
          ...
          <div className="sandbox-footer-actions">
            <button className="btn btn-secondary btn-sm" onClick={handleReset} title="Khôi phục code mẫu ban đầu">
              ↺ Khôi phục
            </button>
            <button
            ...
            >
              {isRunning ? '⏳ Đang chạy...' : '▶️ Chạy Thử (Visible Cases)'}
            </button>
            <button
            ...
            >
              {isRunning ? '⏳ Đang nộp...' : '🚀 Nộp Bài (Kèm Hidden Cases)'}
            </button>
          </div>
        </div>
      </div>
        ...
        <div className="test-results-panel">
          <div className="terminal-header">
            <div className="terminal-tabs">
              <span className="terminal-tab active">TEST RESULTS & CONSOLE</span>
            </div>
            ...
            <div className="terminal-body">
                  ...
                  <div key={idx} className={`test-case-row ${res.passed ? 'pass-row' : 'fail-row'}`}>
                    ...
                    {!res.hidden && !res.passed && (
                      <div className="test-case-diff">
                        ...
                        <div><strong>Actual:</strong> <code>{res.actual}</code></div>
                      </div>
                    )}
                  </div>
                ))}
              ...
              {testOutcome.passed && (
                <div className="test-pass-banner">
                  🎉 Xuất sắc! Đại ca đã vượt qua toàn bộ test case của bài học này. Tiến độ đã được lưu lại!
                </div>
              )}
            </div>
          )}
